...
class Fitness(QMainWindow):
    ...
    def InitUi(self):
        ...
        self.all_workouts = ['High Knee Jumps', "Mountain climbers", "Burpees", "Left high side Plank raises",
                             "Right high side Plank Raises", "Russian twist in v-up position", "Jumping Lunges", "Jumping Jacks",
                             "In and outs", "Side to side jump squats", "Star jump", "Fast feet", "London Bridge"]

        self.all_gifs = ['all_gifs/high knee.gif', 'all_gifs/mountain_climbers.gif', 'all_gifs/burpees.gif',
                         'all_gifs/left high side plank raises.gif', 'all_gifs/right_high_side_plank_raises.gif', 'all_gifs/russian twist.gif', 'all_gifs/jumping_lunges.gif',
                         'all_gifs/jumping_jacks.gif', 'all_gifs/in_and_outs.gif', 'all_gifs/side to side jump squats.gif', 'all_gifs/star jump.gif',
                         'all_gifs/fast feet.gif', 'all_gifs/london_bridge.gif']
    ...
    def start_timer(self):
        if self.minutes_duration.value() == 0 and self.seconds_duration.value() == 0:
        ...
        elif self.break_seconds_duration.value() == 0 and self.break_minutes_duration.value() == 0:
            ...
            for i in range(number_of_drills):
                ...
                if i != 0:
                    self.item = self.all_drills.findItems(
                        self.all_workouts[i-1], QtCore.Qt.MatchRegExp)[0]
                    self.item.setSelected(True)
                    self.all_drills.scrollToItem(self.item)
                    self.item.setBackground(QColor('green'))

                    self.item = self.all_drills.findItems(
                        self.all_workouts[i], QtCore.Qt.MatchRegExp)[0]
                    self.item.setSelected(True)
                    self.all_drills.scrollToItem(self.item)
                    self.item.setBackground(QColor('red'))
                else:
                    self.item = self.all_drills.findItems(
                        self.all_workouts[0], QtCore.Qt.MatchRegExp)[0]
                    self.item.setSelected(True)
                    self.all_drills.scrollToItem(self.item)
                    self.item.setBackground(QColor('red'))
                ...
                self.executor.submit(
                    self.tell, f"Stop! {total_break} seconds rest")
            ...
            self.item = self.all_drills.findItems(
                self.all_workouts[-1], QtCore.Qt.MatchRegExp)[0]
            self.item.setSelected(True)
            self.all_drills.scrollToItem(self.item)
            self.item.setBackground(QColor('green'))
